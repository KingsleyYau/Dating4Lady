...
public class MaterialAppBar extends LinearLayout {
	...
	public void addOverflowButton(String[] menu, MaterialDropDownMenu.OnClickCallback listener, int icon){
		...
		addOverflowButton(menu, listener, icon, size);
	}
	
	public void addOverflowButton(String[] menu, MaterialDropDownMenu.OnClickCallback listener, int icon, Point size) {
		
		final FrameLayout button = createButton(R.id.overflow, "", icon);
	...
	public void setTitle(String title, int color, Drawable drawable, boolean clickable) {
		...
		if (clickable){
			...
			textView.setBackgroundResource(default_touch_feedback);
		}
